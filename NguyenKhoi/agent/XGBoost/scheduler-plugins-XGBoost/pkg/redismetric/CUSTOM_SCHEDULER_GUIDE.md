...
```bash
kind load docker-image localhost:5000/scheduler-plugins/kube-scheduler:v20260531- --name multi-node-demo
```
...
kubectl delete -f scheduler-deploy.yaml
kubectl apply -f scheduler-deploy.yaml



kubectl delete configmap scheduler-config -n kube-system && kubectl create configmap scheduler-config -n kube-system --from-file=scheduler-config.yaml=/home/anhnv_24810310060/lab2/agent/random_forest/scheduler-plugins-random_forest/pkg/redismetric/scheduler-config.yaml
...
kind: Pod
metadata:
  name: custom-scheduler-10
  namespace: scheduler-test
spec:
  schedulerName: my-scheduler-random-forest
  containers:
  - name: pause
...
kubectl delete configmap scheduler-config -n kube-system && kubectl create configmap scheduler-config -n kube-system --from-file=scheduler-config.yaml=/home/anhnv_24810310060/lab2/agent/XGBoost/scheduler-plugins-XGBoost/pkg/redismetric/scheduler-config.yaml
