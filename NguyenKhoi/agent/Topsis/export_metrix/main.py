...
REDIS_HOST = os.getenv("REDIS_HOST", "192.168.24.3")
REDIS_PORT = int(os.getenv("REDIS_PORT", "6379"))
SCYLLA_HOSTS = os.getenv("SCYLLA_HOSTS", "192.168.24.2,192.168.24.3,192.168.24.6").split(",")
...
@contextmanager
def get_redis():
    r = redis.Redis(host=REDIS_HOST, port=REDIS_PORT, db=0, decode_responses=True, socket_timeout=3)
    try:
        yield r
...
def delete_old_data(session, node_id):
    ...
    for row in rows:
        session.execute("DELETE FROM MetricData WHERE node_id = %s AND id = %s", [node_id, row.id])
...
def insert_data(session, agg):
    ...
    session.execute(
        ...
        [
            metric_id, agg["NODE_ID"],
            agg["CPUUsageMax"], agg["CPUUsageMin"], agg["CPUUsageAVG"],
            agg["MemoryUsageMax"], agg["MemoryUsageMin"], agg["MemoryUsageAVG"],
            agg["TailLatencyMax"], agg["TailLatencyMin"], agg["TailLatencyAVG"],
            agg["DiskFreeMax"], agg["DiskFreeMin"], agg["DiskFreeAVG"],
            agg["SwapMemoryMax"], agg["SwapMemoryMin"], agg["SwapMemoryAVG"],
            agg["DiskIOCounterBusyTimeMax"], agg["DiskIOCounterBusyTimeMin"], agg["DiskIOCounterBusyTimeAVG"],
            agg["NetIOCounterDropinMax"], agg["NetIOCounterDropinMin"], agg["NetIOCounterDropinAVG"],
            agg["NetIOCounterDropoutMax"], agg["NetIOCounterDropoutMin"], agg["NetIOCounterDropoutAVG"],
        ]
    )
            ...
            if window_size != WINDOW_SIZE:
                ...
                for attempt in range(5):
                    try:
                        ...
                        logger.warning("ScyllaDB transient (attempt %d): %s", attempt + 1, exc)
