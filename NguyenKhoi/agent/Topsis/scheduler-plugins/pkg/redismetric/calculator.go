...
func CalculateTopNodeFromDB(ctx context.Context, dbConn *sql.DB) (string, error) {
	rows, err := dbConn.QueryContext(ctx, "select * from MetricData LIMIT 5") // Limit to first 5 nodes
	if err != nil {
		return "", err
	}
	defer rows.Close()

	var data_Metrics []MetricData
	for rows.Next() {
		var data MetricData
		err := rows.Scan(
			&data.NODE_ID,
		...
		data_Metrics = append(data_Metrics, data)
	}
	...
	normalizedMetrics := norm_matrix(data_Metrics)
	entropyWeights := Entropy_w(normalizedMetrics)
	decision_matrix := normlised_decision_matrix(entropyWeights, normalizedMetrics)
	worst_best_alternative(decision_matrix)

	// find the best index and format node name
	bestIndex := Score(decision_matrix)
	bestNodeName := fmt.Sprintf("k3d-node-agent-%d", bestIndex)
